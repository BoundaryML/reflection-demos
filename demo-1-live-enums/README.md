...
`pnpm run baml:generate` only after editing `baml_src/` — the CLI toolchain
and the npm bridge must be the same version (see [Toolchain skew](#toolchain-skew)).
...
**Presenter-ready (live only)**, verified against the published toolchain
pinned in the repo-root `BAML_VERSION` (CLI toolchain and npm
`@boundaryml/baml-bridge` at the same version). The model (`claude-haiku-4-5`) correctly
...
- **`baml: command not found`** — install the published CLI and select the
  pinned toolchain: `baml toolchain use "$(cat ../BAML_VERSION)"`.
...
Every symptom below means the CLI toolchain that generated `backend/baml_sdk/`
and the installed npm `@boundaryml/baml-bridge` are different versions. Fix:
`baml toolchain use "$(cat ../BAML_VERSION)"`, `pnpm install`, `pnpm run
baml:generate`.

- **`BAML startup failed: version skew error`** or **`BamlClientError: Bex is
  outdated`** at SDK import — the self-announcing version.
- **`Failed to deserialize BAML bytecode: Unexpected variant tag: N`** at
  import — the client was generated by a newer toolchain than the bridge
  can load.
- **`internal error: entered unreachable code: reflected enum
  ...
  — a bridge older than the reflection fix
  ["runtime type definitions through dispatch" (#4501)][4501].
